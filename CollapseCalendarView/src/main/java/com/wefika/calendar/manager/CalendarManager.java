...
public class CalendarManager {
    ...
    @Nullable
    private LocalDate mOPerationDate;
    ...
    public CalendarManager(@NonNull LocalDate selected, @NonNull State state, @Nullable LocalDate minDate,
                           @Nullable LocalDate maxDate, LocalDate operationDate) {
        this(selected, state, minDate, maxDate, operationDate, null);
    }

    public CalendarManager(@NonNull LocalDate selected, @NonNull State state, @Nullable LocalDate minDate,
                           @Nullable LocalDate maxDate, @Nullable LocalDate operationDate, @Nullable Formatter formatter) {
    ...
    void toggleToWeek(int weekInMonth) {
        Log.e("TESt", "toggleToWeek(int weekInMonth):  " + weekInMonth);
        LocalDate date = mUnit.getFrom().plusDays(weekInMonth * 7);
        toggleFromMonth(date);
    }

    private void toggleFromMonth(LocalDate date) {
//        if (!date.isEqual(date.withDayOfWeek(7))) {
//            date = date.minusWeeks(1).withDayOfWeek(7);
//        }
        date = date.isEqual(date.withDayOfWeek(7)) ? date : date.minusWeeks(1).withDayOfWeek(7);
        Log.e("TESt", "toggleFromMonth(LocalDate date):  " + date);
        setUnit(new Week(date, mToday, mMinDate, mMaxDate, mOPerationDate));
        mUnit.select(mSelected);
        mState = State.WEEK;
    }

    private void toggleFromWeek() {
        Log.e("TESt", "toggleFromWeek() ......");
        setUnit(new Month(mActiveMonth, mToday, mMinDate, mMaxDate, mOPerationDate));
        mUnit.select(mSelected);
    ...
    private void init() {
        if (mState == State.MONTH) {
            setUnit(new Month(mSelected, mToday, mMinDate, mMaxDate, mOPerationDate));
        } else {
            LocalDate from = mSelected.isEqual(mSelected.withDayOfWeek(7)) ? mSelected : mSelected.minusWeeks(1).withDayOfWeek(7);
            setUnit(new Week(from, mToday, mMinDate, mMaxDate, mOPerationDate));
        }
        mUnit.select(mSelected);
    }
