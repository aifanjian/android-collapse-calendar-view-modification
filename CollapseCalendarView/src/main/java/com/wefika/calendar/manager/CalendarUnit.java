...
//                && !mTo.withDayOfWeek(DateTimeConstants.SUNDAY).isBefore(date);
        return !mFrom.withDayOfWeek(1).minusWeeks(1).withDayOfWeek(DateTimeConstants.SUNDAY).isAfter(date)
                && !mTo.withDayOfWeek(DateTimeConstants.SATURDAY).isBefore(date);
    }
