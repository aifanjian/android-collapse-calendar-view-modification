...
public abstract class RangeUnit extends CalendarUnit {
    ...
    protected RangeUnit(@NonNull LocalDate from, @NonNull LocalDate to, @NonNull LocalDate today
            , @Nullable LocalDate minDate, @Nullable LocalDate maxDate, @Nullable LocalDate operationDate) {
        super(from, to, today);
    ...
    public void setOperationDate(@Nullable LocalDate mOperationDate) {
...
//            LocalDate first = date.withDayOfMonth(1).withDayOfWeek(1).minusWeeks(1).withDayOfWeek(DateTimeConstants.SUNDAY);

//            if (!first.withDayOfMonth(1).isEqual(date.withDayOfMonth(1))) {
//                first = first.plusWeeks(1);
//            }
            LocalDate firstDayOfMonth = date.withDayOfMonth(1);
            LocalDate first = firstDayOfMonth.equals(firstDayOfMonth.withDayOfWeek(7)) ? firstDayOfMonth : firstDayOfMonth.minusWeeks(1).withDayOfWeek(7);
            Log.e("TEST", "date:   " + date.toString() + " first:  " + first.toString());
