...
//        mTitleView = (TextView) findViewById(R.id.title);
        ...
        mUpDown = (ImageView) findViewById(R.id.iv_upDown);
    ...
    private void populateDays() {
        ...
        if (!initialized) {
            ...
            if (manager != null) {
                ...
                LocalDate date = LocalDate.now().minusWeeks(1).withDayOfWeek(DateTimeConstants.SUNDAY);
                for (int i = 0; i < 7; i++) {
                    TextView textView = (TextView) layout.getChildAt(i);
                    String text = formatter.getDayName(date).substring(1);
...
//            mTitleView.setText(mManager.getHeaderText());
            ...
            if (mManager.getState() == CalendarManager.State.MONTH) {
                mUpDown.setEnabled(false);
            } else {
                mUpDown.setEnabled(true);
            }
    ...
    private void populateWeekLayout(@NonNull Week week, @NonNull WeekView weekView) {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            if("1".equals(text)){
                text = day.getDate().toString("MMMM");
            }
            dayView.setText(text);
            dayView.setSelected(day.isSelected());
            dayView.setCurrent(day.isCurrent());
            dayView.setIsToady(day.isToday());
            dayView.setIsOperationDay(day.isOperation());
